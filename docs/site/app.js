...
window.initGoogleFoodMap = async function initGoogleFoodMap() {
  state.map = new google.maps.Map(document.getElementById('map'), {
    ...
    styles: [
      ...
      { featureType: 'poi', elementType: 'geometry', stylers: [{ color: '#edf2f7' }] },
      { featureType: 'poi', elementType: 'labels.text.fill', stylers: [{ color: '#64748b' }] },
      { featureType: 'road', elementType: 'geometry', stylers: [{ color: '#ffffff' }] },
      { featureType: 'road.arterial', elementType: 'geometry', stylers: [{ color: '#e2e8f0' }] },
      { featureType: 'road.highway', elementType: 'geometry', stylers: [{ color: '#cbd5e1' }] },
      { featureType: 'transit.station', elementType: 'labels.text.fill', stylers: [{ color: '#0f766e' }] },
  ...
  const res = await fetch('./data/places.json?v=kanidoraku-1');
...
function renderLandmarks() {
  ...
  LANDMARKS.forEach(spot => {
    const marker = new google.maps.Marker({
      ...
      map: state.map,
      title: spot.name,
      label: { text: spot.name, color: '#1e3a5f', fontSize: '12px', fontWeight: '700' },
      icon: {
